    ...
    <AppShell>
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <KpiCard title={px.currentValue} value={perf ? formatMoney(perf.current_value) : "—"} />
          <KpiCard title={px.totalInvested} value={perf ? formatMoney(perf.total_invested) : "—"} />
          <KpiCard
            title={px.gainLossLabel}
            ...
            <div className="space-y-2">
              {alloc.by_wrapper.map((s: any) => (
                <div key={s.asset_class} className="flex items-center gap-4">
                  ...
                  <span className="money text-xs text-slate-500 dark:text-muted-foreground w-20 text-right">
